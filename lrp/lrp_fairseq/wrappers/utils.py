...
def visualize_alti(total_alti, source_sentence, target_sentence, predicted_sentence, word_level, alignment, figsize=(17, 24), all_layers=False):
    ...
    def plot_heatmap_alti(contributions_rollout_layer_np, source_sentence_,
        ...
        trg_contribution = contributions_rollout_layer_np[:, len(source_sentence_):].sum(-1)
        print(contributions_rollout_layer_np[:, :len(source_sentence_)].sum(-1), contributions_rollout_layer_np[:, len(source_sentence_):].sum(-1))
    ...
    if all_layers:
        ...
        target_sentence_ = words_in[words_in.index('</s>')+1:] if word_level else target_sentence
        predicted_sentence_ = words_out if word_level else predicted_sentence
        #print(source_sentence_, source_sentence, words_in)
        #plot_heatmap_alti(contributions_rollout_layer_np, source_sentence_,
        #                    target_sentence_, predicted_sentence_)
