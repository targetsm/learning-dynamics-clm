...
valid_list = [x.split('\t') for x in open(f'./plt/tl/{directory}/valid_list.txt', 'r').readlines()]
valid_list = [(int(y), float(x)) for [x,y] in valid_list]
train_list = [x.split('\t') for x in open(f'./plt/tl/{directory}/train_list.txt', 'r').readlines()]
...
plt.savefig(f'plt/tl/{directory}/train_val_loss.pdf')
...
plt.savefig(f'plt/tl/{directory}/train_val_loss_log.pdf')
