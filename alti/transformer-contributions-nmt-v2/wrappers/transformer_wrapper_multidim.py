...
class FairseqTransformerHub(GeneratorHubInterface):
    ...
    def get_batch(self, split, bs):
        if split not in self.task.datasets.keys():
            self.task.load_dataset(split)
        
        src_tensor = self.task.dataset(split)[self.batch*bs: (self.batch+1)*bs]['source']
        ...
        tgt_tensor = self.task.dataset(split)[self.batch*bs: (self.batch+1)*bs]['target']
        ...
        self.batch +=1
        print(self.batch*bs, (self.batch+1)*bs)
    ...
    def trace_forward(self, src_tensor, tgt_tensor):
        ...
        with torch.no_grad():
            ...
            model_output, encoder_out = self.models[0](src_tensor['src_tokens'].to(torch.int64).to(self.device), src_tensor['src_lengths'].to(torch.int64).to(self.device), tgt_tensor.to(torch.int64).to(self.device), )
    ...
    def normalize_contrib(self, x, mode=None, temperature=0.5, resultant_norm=None):
        ...
        if mode == 'min_max':
            ...
            if resultant_norm == None:
                ...
                x_norm = x + torch.abs(resultant_norm.unsqueeze(2))
    ...
    def __get_contributions_module(self, layer_inputs, layer_outputs, contrib_type, module_name):
        ...
        if pre_layer_norm:
            ...
            print(out_q_pre_ln_th.std(-1).shape)
            ln_std_coef = 1/(out_q_pre_ln_th + eps_ln).std(-1).unsqueeze(-1).unsqueeze(-1) # (batch,src_len,1,1)
            #ln_std_coef = 1/(out_q_pre_ln_th + eps_ln).std(-1).view(1,-1, 1).unsqueeze(-1) # (batch,src_len,1,1)
            print(ln_std_coef.shape)
            print(l_transform(out_qv_pre_ln, w_ln).shape)
            transformed_vectors = l_transform(out_qv_pre_ln, w_ln)*ln_std_coef # (batch,src_len,tgt_len,embed_dim)
            #transformed_vectors = l_transform(out_qv_pre_ln, w_ln)*ln_std_coef.unsqueeze(-1).unsqueeze(-1) # (batch,src_len,tgt_len,embed_dim)
            #transformed_vectors = torch.einsum ('ijkl, im -> ijkl', l_transform(out_qv_pre_ln, w_ln), ln_std_coef)
            print(b_o.shape, w_ln.shape)
            dense_bias_term = l_transform(b_o, w_ln)*ln_std_coef # (batch,src_len,1,embed_dim)
            #dense_bias_term = torch.einsum ('ij, jk -> ik', l_transform(b_o, w_ln).unsqueeze(-1), ln_std_coef)
            ...
            print(torch.dist(out_q_th_2, resultant).item(), 1e-3 * resultant.numel())
    ...
    def get_contribution_rollout(self, src_tensor, tgt_tensor, contrib_type='l1', norm_mode='min_sum', **contrib_kwargs):
        ...
        print(enc_self_attn_contributions.size())
        _, layers, _, _ = enc_self_attn_contributions.size()
        ...
        print(enc_self_attn_contributions_mix.shape)
        relevances_enc_self_attn = enc_self_attn_contributions_mix[:,-1]
        print(relevances_enc_self_attn.shape)
